...
- [ ] P05 Persist     → 可选写入 publish/{platform}/
...
/kb-publish --list
/kb-publish -redbook @notes/.../xxx.md --no-save   # 仅聊天输出，不写文件
```
...
- **独立链路**：不更新 INDEX、不跑 validate-note、不 commit（除非用户另要求）
